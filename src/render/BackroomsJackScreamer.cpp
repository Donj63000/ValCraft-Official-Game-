...
auto load_backrooms_jack_screamer_bmp(
    ...
    if (!stream) {
        return failure(
            "Impossible d'ouvrir l'image de screamer de Jack : " +
            path.string());
    }
    ...
    if (!stream.read(
        ...
        return failure("Lecture incomplete du BMP de Jack");
    }

    if (bytes[0] != static_cast<std::uint8_t>('B') ||
        bytes[1] != static_cast<std::uint8_t>('M')) {
        return failure("La ressource du screamer de Jack n'est pas un BMP");
    }
    ...
    if (dib_size < kBitmapInfoHeaderBytes || planes != 1U ||
        ...
        compression != 0U) {
        return failure(
            "Format BMP non pris en charge pour le screamer de Jack");
    }
    if (width_signed <= 0 || height_signed == 0 ||
        height_signed == std::numeric_limits<std::int32_t>::lowest()) {
        return failure("Dimensions BMP invalides pour le screamer de Jack");
    }
    ...
    if (width > kMaximumScreamerDimension ||
        ...
        return failure("Le BMP de Jack depasse la limite de securite");
    }
    ...
    if (width_size >
        (std::numeric_limits<std::size_t>::max() - 3U) /
            bytes_per_pixel) {
        return failure("Debordement de ligne dans le BMP de Jack");
    }
    ...
    if (height_size != 0U &&
        row_stride >
            (std::numeric_limits<std::size_t>::max() - pixel_offset) /
                height_size) {
        return failure("Debordement de pixels dans le BMP de Jack");
    }
    ...
    if (pixel_offset < kBitmapFileHeaderBytes + dib_size ||
        ...
        return failure("Pixels BMP tronques pour le screamer de Jack");
    }
    if (width_size >
        std::numeric_limits<std::size_t>::max() /
            std::max<std::size_t>(height_size * 4U, 1U)) {
        return failure("Image BMP de Jack trop grande");
    }
