...
private:
    ...
    struct LightingJob {
        ...
        std::vector<std::vector<std::uint8_t>> block_light_buffers;
        std::vector<std::bitset<kChunkSizeX * kChunkSizeZ>> processed_sky_columns;
    ...
    void process_generation_queue(std::size_t budget, WorldWorkStats& stats);
    void process_lighting_queue(std::size_t budget, WorldWorkStats& stats);
    void process_mesh_queue(std::size_t budget, WorldWorkStats& stats);
    ...
    [[nodiscard]] auto lighting_region_contains(const std::vector<ChunkCoord>& region, const ChunkCoord& coord) const noexcept -> bool;
    [[nodiscard]] auto lighting_job_chunk_index(const LightingJob& job, const ChunkCoord& coord) const -> std::optional<std::size_t>;
