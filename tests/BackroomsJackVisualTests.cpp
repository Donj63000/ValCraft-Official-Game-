...
TEST_CASE("Jack debout mesure entre trois metres soixante-dix et trois metres quatre-vingt-dix") {
    const auto parts =
...
TEST_CASE("Jack possede un tricorne une seule botte et une jambe de bois") {
    ...
    CHECK(count_tile(parts, CreatureAtlasTile::ZombieEye) == 2U);
    CHECK(count_tile(parts, CreatureAtlasTile::ZombieTeeth) == 14U);
    CHECK(count_tile(parts, CreatureAtlasTile::ZombieClaw) >= 40U);
}
