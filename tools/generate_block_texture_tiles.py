...
def _distance_to_segment(
    ...
    projection = max(
        0.0,
        min(1.0, ((px - ax) * abx + (py - ay) * aby) / denominator),
    )
    return math.hypot(
        px - (ax + projection * abx),
        py - (ay + projection * aby),
    )


def _make_musket_icon() -> Image.Image:
    result = Image.new("RGBA", (TILE_SIZE, TILE_SIZE), (0, 0, 0, 0))
    output = result.load()

    # Je dessine une silhouette longue et immédiatement lisible, avec la
    # crosse en noyer sous le canon, les bandes et la platine métalliques.
    for y in range(TILE_SIZE):
        for x in range(TILE_SIZE):
            px = float(x) + 0.5
            py = float(y) + 0.5
            stock = _distance_to_segment(px, py, 1.7, 12.5, 8.7, 7.2)
            barrel = _distance_to_segment(px, py, 6.8, 6.4, 14.5, 2.7)
            ramrod = _distance_to_segment(px, py, 7.5, 7.4, 13.8, 4.3)
            butt = ((px - 2.2) / 2.0) ** 2 + ((py - 12.0) / 2.4) ** 2
            lock = ((px - 7.0) / 1.35) ** 2 + ((py - 7.1) / 1.10) ** 2
            band = any(
                math.hypot(px - bx, py - by) <= 0.85
                for bx, by in ((8.8, 5.7), (11.2, 4.6), (13.3, 3.6))
            )

            if barrel <= 0.72 or ramrod <= 0.30:
                shine = 26 if (x + y) % 4 == 0 else 0
                output[x, y] = (
                    _clamp_byte(112 + shine),
                    _clamp_byte(119 + shine),
                    _clamp_byte(122 + shine),
                    255,
                )
            if stock <= 1.35 or butt <= 1.0:
                grain = ((x * 7 + y * 11) % 13) - 6
                output[x, y] = (
                    _clamp_byte(112 + grain * 2),
                    _clamp_byte(67 + grain),
                    _clamp_byte(34 + grain * 0.5),
                    255,
                )
            if lock <= 1.0 or band:
                highlight = 24 if x <= 9 else 8
                output[x, y] = (
                    _clamp_byte(166 + highlight),
                    _clamp_byte(126 + highlight * 0.7),
                    _clamp_byte(52 + highlight * 0.3),
                    255,
                )
...
def _render_header(tiles: Dict[str, Image.Image]) -> bytes:
...
def _write_header(output_path: Path, generated: bytes) -> None:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verifie que le header versionne correspond exactement a la recette sans le reecrire.",
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    generated = _render_header(tiles)
    if args.check:
        if args.preview is not None:
            sys.stderr.write("--preview ne peut pas etre utilise avec --check.\n")
            return 2
        try:
            existing = args.output.read_bytes()
        except OSError as error:
            sys.stderr.write(f"Header Legacy absent ou illisible : {error}\n")
            return 1
        if existing != generated:
            sys.stderr.write(
                f"Le header Legacy {args.output} n'est pas synchronise avec ses recettes.\n"
            )
            return 1
        print(
            f"Header Legacy deterministe verifie : {args.output} "
            f"({len(generated)} octets, {len(OUTPUT_ORDER)} tuiles)."
        )
        return 0

    _write_header(args.output, generated)
    if args.preview is not None:
